...
public class DeleteRowsNodeModel extends NodeModel {
	...
	private void run(final ExecutionContext exec, final RDFService service, final RowInput rowInput)
		...
		final String uri = m_settings.getURI().trim();
		...
		while ((row = rowInput.poll()) != null) {
			...
			final String graphName = SemanticWebUtility.getGraphName(row, graphName_idx, uri, service,
			...
			if (subCol == null && preCol == null && objCol == null && graphName_idx >= 0) {
				...
				filter.setLength(0);
					...
					if (!sub.isMissing()) {
					filter.append("?sub = <" + SemanticWebUtility.getResourceName(uri, sub.toString()) + ">");
					...
					filter.append("?pre = <" + SemanticWebUtility.getResourceName(uri, pre.toString()) + ">");
						predecessorExists = true;
					}
				...
				delete.append(
						" WHERE " + service.getQueryStatement(" ?sub ?pre ?obj . FILTER (" + filter + ")", graphName));
